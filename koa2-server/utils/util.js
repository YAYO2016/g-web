const util = {
        TypeFn: {
            ...
            checkStr(str, type) { // 判断字符串的类型是否正确
                switch (type) {
                    case 'phone':   //手机号码
        ...
        DateFn: {
             ...
             * 格式化时间
             *
             * @param  {time} 时间 参数只能是时间戳或者时间戳字符串
             * @param  {cFormat} 格式
             * @return {String} 字符串
            ...
            /*获取某月有多少天  getMonthOfDay('2018/2/25')-->29*/
            getMonthOfDay(time) {
        ...
        ArrayFn: {
            ...
            /*数组对象 差集*/
            arrObjDiff(arrA, arrB, key) {
                let new_tmp = new Array();
                for (let i = 0; i < arrA.length; i++) {
                    let flag = true;
                    for (let j = 0; j < arrB.length; j++) {
                        if (arrA[i][key] == arrB[j][key]) {
                            flag = false;
                        }
                    }
                    if (flag) {
                    ...
                    parents.forEach((parent) => {
                            ...
                            childrens.forEach((current, index) => {
                                    ...
                                    if (current[belongId] === parent[id]) {
                                        ...
                                        typeof parent[keyChildrenName] !== 'undefined' ? parent[keyChildrenName].push(current) : parent[keyChildrenName] = [current]
                                    }
                                }
                            )
                        }
        ...
        NumberFn: {
             ...
             * @returns {string}
             * 示例 numberFormat(1000.123, 2) =>  1,000.13
             */
            ...
            //js的计算是存在误差的，为了避免这样的现象，需要对计算特殊处理
            //js 加法
            accAdd(arg1, arg2) {
                let r1, r2, m;
                try {
        ...
        StorageFn: {
            ...
            /* 获取当前登录人角色数组 */
            getUserRoleMap() {
                let userInfo = this.getLocal('userInfo');
                if (!userInfo || !userInfo.userRoles) return null;
                let resultMap = [];
                userInfo.userRoles.map(role => {
                    resultMap.push({code: role.roleId, name: role.roleName});
                });
                return resultMap;
            }
            ,
            ...
            /* 校验当前登录人的权限 */
            checkPrivileges(needPrivileges) {
                let userInfo = this.getLocal('userInfo');
        ...
        OtherFn: {
            ...
            sleep(numberMills) {
                let now = new Date();
                let exitTime = now.getTime() + numberMills;
                while (true) {
                    now = new Date();
